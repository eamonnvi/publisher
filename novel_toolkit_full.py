...
========================================
Clean reference version 2025-08-10 • v5.4

What's new in v5.4
------------------
- Pricing estimates:
  --price                    add per-section and total cost estimates
  --fx-rate FLOAT            multiply USD costs (e.g., 0.78 ≈ GBP)
  --assume-cached-percent N  percent of input tokens billed at cached-input rate
- Profiling expanded: profile.csv now includes cost columns.
- Batch dry-run retained: build batch_input.jsonl without submitting.
...
- Batch API: --batch-submit / --batch-fetch (50% discount applied in estimates).
- Continuity mode supported in sync path; batch uses a fixed prior.
"""
...
# USD per 1M tokens (approx; update if OpenAI changes posted rates)
# Tuple = (input, cached_input, output)
PRICING_USD = {
    "gpt-5":        (1.25, 0.125, 10.00),
    "gpt-5-mini":   (0.25, 0.025, 2.00),
    "gpt-5-nano":   (0.05, 0.005, 0.40),
    "gpt-4.1":      (5.00, 1.25, 20.00),
    "gpt-4.1-mini": (0.60, 0.30, 2.40),
}

def _lookup_rates(model: str) -> tuple[float, float, float]:
    key = model
    if key not in PRICING_USD:
        if "gpt-5" in model:
            if "mini" in model: key = "gpt-5-mini"
            elif "nano" in model: key = "gpt-5-nano"
            else: key = "gpt-5"
        elif "gpt-4.1" in model:
            key = "gpt-4.1-mini" if "mini" in model else "gpt-4.1"
    return PRICING_USD.get(key, (0.0, 0.0, 0.0))

def estimate_cost_usd(
    model: str,
    tokens_in: int,
    tokens_out: int,
    cached_percent: float = 0.0,
    is_batch: bool = False,
) -> tuple[float, float, float, float]:
    """
    Returns (cost_input, cost_output, total_cost, effective_input_rate_used)
    cached_percent is 0..100. is_batch applies 50% discount to both legs.
    """
    rate_in, rate_cached, rate_out = _lookup_rates(model)
    if rate_in == 0.0 and rate_out == 0.0:
        return (0.0, 0.0, 0.0, rate_in)
    cached_frac = max(0.0, min(100.0, cached_percent)) / 100.0
    uncached_in = tokens_in * (1.0 - cached_frac)
    cached_in   = tokens_in * cached_frac
    disc = 0.5 if is_batch else 1.0
    cost_in  = ((uncached_in / 1_000_000.0) * rate_in + (cached_in / 1_000_000.0) * rate_cached) * disc
    cost_out =  (tokens_out / 1_000_000.0) * rate_out * disc
    return (cost_in, cost_out, cost_in + cost_out, rate_in * disc)
...
def call_llm(
    ...
    def use_responses(m: str, eng: str) -> bool:
        ...
        if eng == "chat":
            return False
        return m.startswith("gpt-5")  # auto for GPT-5 family
    ...
    def responses_call(m: str):
        ...
        if not m.startswith("gpt-5"):  # avoid temp for gpt-5 responses unless you know it's supported
...
def main() -> None:  # noqa: C901
    ...
    p.add_argument("--price", action="store_true",
                   help="Estimate cost per section and totals (USD by default, scaled by --fx-rate)")
    p.add_argument("--fx-rate", type=float, default=1.0,
                   help="Currency multiplier applied to USD (e.g., 0.78 for GBP)")
    p.add_argument("--assume-cached-percent", type=float, default=0.0,
                   help="Percent of input tokens billed at cached-input price (0–100)")
    ...
    if args.batch_submit:
        ...
        for idx, (heading, body) in enumerate(sections, start=1):
            prior_json = json.dumps(entity_memory, ensure_ascii=False) if (args.mode == "continuity" and entity_memory) \
                         else ("[]" if args.mode == "continuity" else None)
    ...
    if args.batch_fetch:
        ...
        # Totals
        run_tok_in = run_tok_out = 0
        run_cost_in = run_cost_out = 0.0
        ...
        for idx, (heading, _body) in enumerate(sections, start=1):
            ...
            if args.profile:
                ...
                if prof_writer:
                    if args.price:
                        c_in, c_out, c_tot, _ = estimate_cost_usd(
                            args.model, tokens_in, tokens_out,
                            cached_percent=args.assume_cached_percent,
                            is_batch=True
                        )
                        fx = args.fx_rate
                        prof_writer.writerow([idx, heading, "batch", args.model, tokens_in, tokens_out, 0.0,
                                              round(c_in*fx, 6), round(c_out*fx, 6), round(c_tot*fx, 6),
                                              ("USD" if fx==1.0 else f"USD*x{fx}")])
                        run_tok_in  += tokens_in
                        run_tok_out += tokens_out
                        run_cost_in += c_in
                        run_cost_out+= c_out
                    else:
                        prof_writer.writerow([idx, heading, "batch", args.model, tokens_in, tokens_out, 0.0,
        ...
        if prof_fh: prof_fh.close() if prof_fh else None

        if md_fh and args.format in {"md", "both"}:
            prepend_cli_metadata_to_output(md_path, args, args.mode)

        if args.profile and args.price:
            fx = args.fx_rate
            tot = (run_cost_in + run_cost_out) * fx
            print(f"[cost] tokens_in={run_tok_in:,} tokens_out={run_tok_out:,} "
                  f"cost_input={run_cost_in*fx:.4f} cost_output={run_cost_out*fx:.4f} total={tot:.4f} "
                  f"currency={'USD' if fx==1.0 else f'USD*x{fx}'}")

        print(f"[ok] Wrote batch results to {out_dir.resolve()}")
        return

    # -------------------------------------------------------------------------
    # Synchronous path (original behaviour) with profiling & pricing
    # -------------------------------------------------------------------------
    ...
    try:
                ...
                if prof_writer:
                    if args.price:
                        c_in, c_out, c_tot, _ = estimate_cost_usd(
                            ...
                            total_in, tokens_out,
                            cached_percent=args.assume-cached_percent if False else args.assume_cached_percent,
                            is_batch=False
                        )
                        fx = args.fx_rate
                        prof_writer.writerow([
                            ...
                            chosen_endpoint["model"] or args.model,
                            total_in, tokens_out, round(dt, 3),
                            round(c_in*fx, 6), round(c_out*fx, 6), round(c_tot*fx, 6),
                            ("USD" if fx==1.0 else f"USD*x{fx}")
                        ])
                        run_tok_in  += total_in
        ...
        if json_fh:
            json_fh.close()
        if prof_fh:
            prof_fh.close() if prof_fh else None
